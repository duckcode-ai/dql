...
export function DbtFirstModelingPage() {
  ...
  return (
    ...
    >
      ...
      <div style={{ minHeight: 28, flexShrink: 0, borderTop: `1px solid ${t.headerBorder}`, background: t.headerBg, display: 'flex', alignItems: 'center', gap: 14, padding: '0 12px', color: t.textMuted, fontSize: 9.5 }}><span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}><span style={{ width: 6, height: 6, borderRadius: 999, background: '#2e8b57' }} />dbt synced · {Object.keys(data.dbtProvenance.nodes).length} models</span><span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}><span style={{ width: 6, height: 6, borderRadius: 999, background: t.accent }} />{domainRelationships.filter((relationship) => relationship.validation?.status === 'passed').length} relationships proven</span><span style={{ marginLeft: 'auto' }}>Saved to your workspace · review changes before sharing</span></div>
      {editor && (
        <ModelingEditor
          editor={editor}
          data={data}
