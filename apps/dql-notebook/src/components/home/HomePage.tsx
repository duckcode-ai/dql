...
function Chapter({ number, title, subtitle, children, right = false }: { number: string; title: string; subtitle: string; children: React.ReactNode; right?: boolean }) { return <section className={`dql-chapter ${right ? 'dql-chapter-right' : ''}`}><header><span>{number}</span><div><b>{title}</b><small>{subtitle}</small></div></header>{children}</section>; }
