...
The OSS release candidate is synchronized for `1.12.1`; npm `latest`, registry
visibility, and clean local/global CLI installs remain release gates before it
becomes the verified current release. The tracked `PERF-001` scale fixture has
...
Maintainer approval will record these conditions as disclosed `v1.12.1` OSS release
