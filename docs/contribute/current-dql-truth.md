...
| Current OSS release | `1.13.0` across all 19 publishable packages and the starter CLI range                                   | Release manifests and starter CLI range on `main`                                                                        |
| npm publication     | All 19 packages report `version=1.13.0` and `dist-tags.latest=1.13.0`                                  | Package-by-package `npm view` audit                                                                                       |
| Published CLI       | Fresh project-local and global installs both reported `dql 1.13.0`                                     | `node scripts/smoke-cli-install.mjs @duckcodeailabs/dql-cli@1.13.0`                                                      |
| Release tag         | `v1.13.0` identifies the final Git release baseline                                                    | `git rev-parse v1.13.0`; `git ls-remote --tags origin refs/tags/v1.13.0`                                                 |
...
useful historical release evidence. Their latest `1.13.0` entries are release
